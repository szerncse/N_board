...
export default function FlatformCom(){
    ...
    const fetchData = async ()=>{
      ...
      try{
        ...
        if(res.ok){
          ...
          const data:MemberType[] = result.data.platformResult;
          const userCount: { [key: string]: number } = {};
          data.forEach(item => {
            if (dateType === 'today' && new Date(item.date).toDateString() === new Date().toDateString()) {
              userCount[item.platform] = (userCount[item.platform] || 0) + item.platform_count;
